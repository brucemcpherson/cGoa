  ...
  goaApp.jwt = {
    ...
    */
    generate: function (pockage, tokenPacket) {
      ...
      // generate the jwt
      var jwt = Utils.encodeB64(JSON.stringify(tokenPacket.header)) + "." +
        Utils.encodeB64(JSON.stringify(tokenPacket.claims));

      // now sign it 
      var signed = Utils.encodeB64(Utilities.computeRsaSha256Signature(jwt, pockage.private_key));
    ...
    */
    makeTokenRequest: function (pockage, impersonate, timeout) {
      ...
      // request a new one
      var result = UrlFetchApp.fetch(goaApp.getServicePackage(pockage).tokenUrl, {
        method: "POST",
        muteHttpExceptions: true,
